...
vmaggies=ut.mag2maggies(v06)
rmaggies=ut.mag2maggies(r06)
...
iinvervar=ut.invariance(imaggies,i06err)
...
def make_k_lookup_tab(karray,low_range,high_range,n_elem):
	kcorrect.load_templates()
	kcorrect.load_filters('/home/lhunt/programs/kcorrect/data/templates/Lum_Func_Filters.dat')
	zref=linspace(low_range,high_range,n_elem)
	for i in range(0,len(karray)):
		for j in range(0,(len(karray)-1)/2)
		for j in range(0,len(zref)):
			coeff=kcorrect.fit_nonneg(zref[j],karray[i][1:6],karray[i][6:12])
	...
	for j in range(0,len(LumFuncWhere[i])):
		...
		for k in range(0,len(LumFuncWhere[i][j])):
			...
			if zarr[LumFuncWhere[i][j][k]]<=0.16:
				tempM=M[LumFuncWhere[i][j][k]]
				tempz=zarr[LumFuncWhere[i][j][k]]
				while np.logical_or(tempM>MBinUp[j],tempz<=zbinup[i]):
					coeff=kcorrect.fit_nonneg(tempz,karray[LumFuncWhere[i][j][k]][1:6],karray[LumFuncWhere[i][j][k]][6:12])
					m=kcorrect.reconstruct_maggies(coeff)
					tempM=b06[LumFuncWhere[i][j][k]]-5*(np.log10(100000*cosmo.luminosity_distance(tempz).value))-(-2.5*np.log10(m[1]/b06rm0[LumFuncWhere[i][j][k]])+5*np.log10(0.6932))
					tempz=tempz+0.01	
				tempz=tempz-0.01
				while np.logical_or(tempM>MBinUp[j],tempz<=zbinup[i]):
					coeff=kcorrect.fit_nonneg(tempz,karray[LumFuncWhere[i][j][k]][1:6],karray[LumFuncWhere[i][j][k]][6:12])
				...
				tempM=M[LumFuncWhere[i][j][k]]
				tempz=zarr[LumFuncWhere[i][j][k]]
				while np.logical_or(tempM<MBinLow[j],tempz>=zbinlow[i]):
					coeff=kcorrect.fit_nonneg(tempz,karray[LumFuncWhere[i][j][k]][1:6],karray[LumFuncWhere[i][j][k]][6:12])
					m=kcorrect.reconstruct_maggies(coeff)
					tempM=b06[LumFuncWhere[i][j][k]]-5*(np.log10(100000*cosmo.luminosity_distance(tempz).value))-(-2.5*np.log10(m[1]/b06rm0[LumFuncWhere[i][j][k]])+5*np.log10(0.6932))
					tempz=tempz-0.01
...
for i in range(0,len(sortweighting)):
	for j in range(0,len(sortweighting[i])):
		...
		print(zuse1w,zuse2w,zuse3w,zuse4w)
		if (zuse1w+zuse2w)!=0:
		...
		if (zuse1w+zuse2w+zuse3w)!=0:
			...
			TSR=1
		WeightArray[sortweighting[i][j]]=SSR\\\\\\\\\\\\\\\\\\\\\\\\\\\4
		print(SSR,TSR)
...
if args.zbin>2 and args.zbin<=4:
	...
	i=0	
	if args.overplot:
		phi=np.array([0.001460,0.00962,0.001507,0.00907])
		Mparam=np.array([-20.09,-20.45,-20.36,-20.87])
		alpha2=np.array([-1.15,-1.22,-1.12,-1.33])
	for i,a in enumerate(axes.flatten(),start=1):
